...
bool Dijkstra::findPath() {
    ...
    PriQueueNode* openlist_node = createPriQueueNode(point_start_,nullptr,0.0);
    ...
    while (!open_list_.empty()) {
        ...
        if(openlist_node->pos == point_goal_) {
            while(openlist_node->pos != point_start_) {
                circle(mat_temp, Point(openlist_node->pos.first, openlist_node->pos.second), 4, Scalar(0, 255, 0), -1);
